...
use otr_processor::model::{calc_rankings, get_global_rank, get_country_rank};
use otr_processor::model::structures::mode::Mode;
use otr_processor::model::structures::player_rating::PlayerRating;
...
pub fn calc_rankings_old(existing_ratings: &mut [PlayerRating], country_hash: &HashMap<i32, Option<String>>) {
    for player in existing_ratings.iter() {
        ...
        let _country_rank = get_country_rank (
            &player.rating.mu,
            &player.player_id,
            &country_hash,
            &existing_ratings,
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let mut ratings = Vec::with_capacity(
        AMOUNT_OF_COUNTRIES * AMOUNT_OF_USERS_IN_ONE_COUNTRY
    );
